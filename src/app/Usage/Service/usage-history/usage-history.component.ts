...
@Component({
  selector: 'app-usage-service-history',
...
export class UsageHistoryComponent implements OnInit, AfterContentChecked {
  ...
  ngOnInit(): void {
    this.csvFileName = 'Usage Transaction - ' + this.ServiceID;
    this.getPermission();
  }
  ...
  private formatPermissions(value: Permission[]): void {
    this.permissions = value.map(s => s.Resource.replace('/UsageTransactions/Services', "").replace('/', "") as PermissionType);
  }
  private async getPermission(): Promise<void> {
    await this.loading.present();
    this.globService.getAuthorization('/UsageTransactions/Services', true)
  ...
  async getUsageHistory() {
    ...
    this.usageHistoryService.getUsageHistory(reqData, this.ServiceReference)
      .pipe(takeUntil(this.unsubscribeAll$))
      .subscribe({
        next: async (result) => {
          await this.loading.dismiss();
