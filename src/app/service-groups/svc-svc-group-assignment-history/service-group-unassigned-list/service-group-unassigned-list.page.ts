...
import { ServiceGroupUnassignedListService } from './services/service-group-unassigned-list.service';
import { GlobalService } from 'src/services/global-service.service';

@Component({
  selector: 'app-service-group-unassigned-list',
  templateUrl: './service-group-unassigned-list.page.html',
  styleUrls: ['./service-group-unassigned-list.page.scss'],
})
export class ServiceGroupUnassignedListPage implements OnInit {

  @Input() serviceId: any = '';

  @ViewChild('SvcSvcGroupUnAssignGrid') SvcSvcGroupUnAssignGrid: jqxGridComponent;
  ...
  constructor(
    ...
    private sGService: ServiceGroupUnassignedListService,
  ...
  selectRow() {
    this.selectIndex = this.SvcSvcGroupUnAssignGrid.getselectedrowindex();
    this.selectedData = this.SvcSvcGroupUnAssignGrid.getrowdata(this.selectIndex);
  ...
  exportData() {
    this.SvcSvcGroupUnAssignGrid.exportview('xlsx', 'Servic Service Group Unassigned List');
  }
  ...
  setGridData() {
    ...
    this.SvcSvcGroupUnAssignGrid.updatebounddata();
    this.source.localdata.length = 0;
    if (this.availDeleteState) {
      this.SvcSvcGroupUnAssignGrid.clearselection();
    }
    this.availDeleteState = false;
  }
