...
@Component({
    selector: 'app-service-attribute-charging',
...
export class AttributeChargingComponent implements OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.planDefinitions?.currentValue) {
      this.setOptionList();
      this.csvFileName = this.tranService.instant('ServicePlanAttributeCharge') + `_${this.planDefinitions.Name}`;
    }
  }

  selectOption(index: number): void {
    console.warn('data =>', this.optionsList[index].AttributeCharges)
