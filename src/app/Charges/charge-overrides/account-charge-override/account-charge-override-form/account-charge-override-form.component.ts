...
@Component({
  selector: 'app-account-charge-override-form',
  templateUrl: './account-charge-override-form.component.html',
  styleUrls: ['./account-charge-override-form.component.scss'],
  providers: [DatePipe]
})
export class AccountChargeOverrideFormComponent implements OnInit {
  @Input() ContactCode: string = '';
  @Input() ChargeOverrideToUpdate: ChargeOverride;
  @Input() EditMode: string = 'New';
  @Output('AccountChargeOverrideFormComponent') AccountChargeOverrideFormComponent: EventEmitter<string> = new EventEmitter<string>();
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<AccountChargeOverrideFormComponent>,
  ...
  ngOnInit() {
    if (Object.keys(this.dlgData).length > 0) {
    ...
    if (this.EditMode == 'New') {
      this.getPermission('/Accounts/Charges/Overrides/New');
    } else if (this.EditMode == 'Update') {
      this.getPermission('/Accounts/Charges/Overrides/Update');
    } else if (this.EditMode == 'View') {
      this.groupForm.disable();
      this.getPermission('/Accounts/Charges/Overrides/Details');
    }
  }
  ...
  async getPlanDefinitions(searchString: string) {
    ...
    this.showPlanSpinner = true;
    this.accountService.getPlanDefinitions(this.ContactCode, this.globService.convertRequestBody(reqData)).subscribe(async (result: any) => {
      this.showPlanSpinner = false;
  ...
  getChargeDefinitions(searchString: string) {
    ...
    this.accountService.getChargeDefinitions(this.ContactCode, this.globService.convertRequestBody(reqData)).subscribe(async (result: any) => {
  ...
  async createCharge(reqData: ChargeOverride) {
    await this.spinnerService.loading();
    this.accountService.createChargeOverride(this.ContactCode, this.globService.convertRequestBody(reqData)).subscribe(async (result: any) => {
      await this.spinnerService.end();
      if(this.isModalDlg){
        this.dialogRef.close('ok');
      }else{
        this.AccountChargeOverrideFormComponent.emit('go-back');
      }
    }, async (error: any) => {
      await this.spinnerService.end();
      this.tranService.matErrorMessage(error, (title, button, content) => {
