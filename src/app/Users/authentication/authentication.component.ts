import { Component, EventEmitter, Input, OnChanges, Inject, OnDestroy, OnInit, Output, SimpleChanges } from '@angular/core';
...
export class AuthenticationComponent implements OnInit {
  ...
  @Output('AuthenticationComponent') public AuthenticationComponent: EventEmitter<any> = new EventEmitter<any>();
  ...
  async ngOnInit() {  
    ...
    this.globService.getAuthorization('/Contacts/Authentication')
      .subscribe({
        next: async (_result) => {
  ...
  initComponent(){
    this.authService.getAuthenticationInfo(this.ContactCode).subscribe(async (result: any) => {
        await this.loading.dismiss();
        if(result){
          this.groupForm.get('Email').setValue(result.Email);
          this.groupForm.get('MobilePhone').setValue(result.Mobile);
          this.groupForm.get('LoginId').setValue(result.LoginId);          
          this.curAuthInfo = result;
          this.pswLoginState = result.ChangePasswordOnFirstLogin;          
          ...
          this.groupForm.get('LoginId').setValue(this.ContactCode);          
          this.setValidFormCtrl('LoginId');
        }
  ...
  closeComponent() {
    this.AuthenticationComponent.emit('close');
  }
  ...
  async checkLoginId() {
    ...
    this.authService.checkLoginId(this.globService.getFormValue(this.groupForm.get('LoginId').value), this.ContactCode).subscribe(async (result: any) => {
  ...
  async checkEmailValidate() {
    ...
    this.authService.checkEmailFormatValidate(this.globService.getFormValue(this.groupForm.get('Email').value)).subscribe(async (result: any) => {
      ...
      if (!formatValidateResult.valid) {
        ...
        this.authService.checkEmailValidate(this.globService.getFormValue(this.groupForm.get('Email').value), this.ContactCode).subscribe(async (result: any) => {
          this.spinner.email = false;
          const convResult = this.globService.ConvertKeysToLowerCase(result);
          if (convResult.used) {
  ...
  async checkMobileValidate() {
    ...
    this.authService.checkMobileFormatValidate(val).subscribe(async (result: any) => {
      ...
      if (!convMobileValidateResult.valid) {
        ...
        this.authService.checkMobileValidate(val, this.ContactCode).subscribe(async (result: any) => {
          this.spinner.mobile = false;
          const convResult = this.globService.ConvertKeysToLowerCase(result);    
          if (convResult.used) {
  ...
  getSaveBtnState(){    
    return (this.saveAvailableCall('email') && this.saveAvailableCall('loginId')) || !this.groupForm.valid;
  }
  
  saveAvailableCall(type) {
  ...
  parseData() {
    // if (this.groupForm.valid) {
    //   if (this.groupState === 'edit') {
    //     this.AuthenticationComponent.emit({
    ...
    //       MFAEnabled: this.enableTwoFactor,
    //     });
    //   } else {
    //     if (this.groupForm.get('TemporaryPassword').value) {
    //       this.AuthenticationComponent.emit({
    ...
    //   }
    // } else {
    //   this.AuthenticationComponent.emit(false);
    // }
  }
  ...
  async saveAuth(){
    ...
    await this.loading.present();
    this.authService.saveAuthenticateInfo(reqData, this.ContactCode, 'Customer').subscribe(async (result: any) => {
      await this.loading.dismiss();      
      this.tranService.errorToastMessage('AuthenticationSaved');
      this.closeComponent();
    }, async (error: any) => {
      await this.loading.dismiss();
      this.tranService.errorMessage(error);
