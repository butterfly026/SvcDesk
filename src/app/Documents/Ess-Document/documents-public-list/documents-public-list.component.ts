...
import { DocumentsPublicService } from './services/document-public.service';

@Component({
  selector: 'app-documents-public-list',
  templateUrl: './documents-public-list.component.html',
  styleUrls: ['./documents-public-list.component.scss'],
})
export class DocumentsPublicListComponent implements OnInit {
  @Input() viewType: string = '';
  @Input() currentService: string = '';
  @ViewChild('docPublicGrid') docPublicGrid: jqxGridComponent;
  @Output('docPublicComponent') docPublicComponent: EventEmitter<string> = new EventEmitter<string>();
  ...
  constructor(
    ...
    private docService: DocumentsPublicService,
  ...
  async getGroupList() {
    ...
    this.docService.getDocumentPublicList(reqPara).subscribe(async (result: any[]) => {
      ...
      this.docPublicComponent.emit('setMinHeight');
    }, async (error: any) => {
  ...
  exportExcel() {
    this.docPublicGrid.exportview('xlsx', 'Public Documents list');
  }

  exportPdf() {
    this.docPublicGrid.exportdata('pdf', 'Public Documents list');
  }
  ...
  selectRow(event) {
    if (typeof (event.args.row.bounddata.id) !== 'undefined' && event.args.row.bounddata.id !== null) {
      ...
      this.selectedData = this.docPublicGrid.getrowdata(this.selectIndex);
    } else {
  ...
  setGridWidth() {
    ...
    if (this.docPublicGrid) {
      ...
      if (typeof (gridContent) === 'undefined' || gridContent === null) {
        ...
        for (let list of this.docPublicGrid.attrColumns) {
  ...
  setGridData() {
    ...
    this.docPublicGrid.updatebounddata();
    this.source.localdata.length = 0;


    this.gridRecords = this.docPublicGrid.attrSource;
        ...
        switch (list.datafield) {
          case 'id':
            if (columnsMaxWidth.id < result.length) {
              this.docPublicGrid.setcolumnproperty("id", "width", result.length * 8 + 20);
            } else {
              this.docPublicGrid.setcolumnproperty("id", "width", columnsMaxWidth.id * 8 + 10);
            }
            break;
          case 'label':
            if (columnsMaxWidth.label < result.length) {
              this.docPublicGrid.setcolumnproperty("label", "width", result.length * 8 + 20);
            } else {
              this.docPublicGrid.setcolumnproperty("label", "width", columnsMaxWidth.label * 8 + 10);
            }
            break;
          case 'name':
            if (columnsMaxWidth.name < result.length) {
              this.docPublicGrid.setcolumnproperty("name", "width", result.length * 8 + 20);
            } else {
              this.docPublicGrid.setcolumnproperty("name", "width", columnsMaxWidth.name * 8 + 10);
            }
            break;
          // case 'type':
          //   if (columnsMaxWidth.type < result.length) {
          //     this.docPublicGrid.setcolumnproperty("type", "width", result.length * 8 + 20);
          //   } else {
          //     this.docPublicGrid.setcolumnproperty("type", "width", columnsMaxWidth.type * 8 + 10);
          //   }
          //   break;
          case 'updated':
            if (columnsMaxWidth.updated < result.length) {
              this.docPublicGrid.setcolumnproperty("updated", "width", result.length * 8 + 20);
            } else {
              this.docPublicGrid.setcolumnproperty("updated", "width", columnsMaxWidth.updated * 8 + 10);
            }
            break;
          case 'updatedby':
            if (columnsMaxWidth.updatedby < result.length) {
              this.docPublicGrid.setcolumnproperty("updatedby", "width", result.length * 8 + 20);
            } else {
              this.docPublicGrid.setcolumnproperty("updatedby", "width", columnsMaxWidth.updatedby * 8 + 10);
            }
            break;
          case 'created':
            if (columnsMaxWidth.created < result.length) {
              this.docPublicGrid.setcolumnproperty("created", "width", result.length * 8 + 20);
            } else {
              this.docPublicGrid.setcolumnproperty("created", "width", columnsMaxWidth.created * 8 + 10);
            }
            break;
          case 'created_by':
            if (columnsMaxWidth.created_by < result.length) {
              this.docPublicGrid.setcolumnproperty("created_by", "width", result.length * 8 + 20);
            } else {
              this.docPublicGrid.setcolumnproperty("created_by", "width", columnsMaxWidth.created_by * 8 + 10);
            }
            break;
          default:
            break;
  ...
  gotoNew() {
    ...
    this.docPublicComponent.emit('add');
  }
  ...
  goToService() {
    ...
    this.currentType.emit(this.showType);
    this.docPublicComponent.emit('sitesservice' + this.groupList[index].id);
  }
  ...
  selectDocument(id) {
    this.docPublicComponent.emit('selectdocumentindex' + id + 'selectdocumentindex' + this.showType);
  }
