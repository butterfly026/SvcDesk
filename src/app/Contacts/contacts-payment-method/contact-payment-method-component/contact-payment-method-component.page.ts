...
import {TillComponent} from '../../../payment-methods/till/till.component'
...
@Component({
  selector: 'app-contact-payment-method-component',
  templateUrl: './contact-payment-method-component.page.html',
  styleUrls: ['./contact-payment-method-component.page.scss'],
})
export class ContactPaymentMethodComponentPage implements OnInit,AfterViewInit  {
  ...
  BankHistoryState: string = '';
    ...
    private cPMLNewService: ContactPaymentMethodService,
    ...
    this.stepForm1.get('cardNumber').valueChanges.pipe(debounceTime(1000)).subscribe(async (result) => {
      if(result)
        this.cardNumberKeydown();
    })
  ...
  ngAfterViewInit() {
    //this.istillLoaded=true;
    //this.tillPayment.createForm();
  }
  ...
  overTableBank(index) {
    const element = document.getElementById('customeTableBodyBank' + index);
    const elementDetail = element.getElementsByClassName('custom-table-body-td2');
    for (let i = 0; i < elementDetail.length; i++) {
      elementDetail[i].classList
        .add('jqx-fill-state-hover', 'jqx-fill-state-hover-' + this.globService.themeColor);
    }
  ...
  async getBothList() {
    ...
    this.cPMLNewService.PaymentMethods(this.ContactCode).subscribe(async (result: ContactPaymentMethods[]) => {
      await this.loading.dismiss();
      
      if (result === null) {
        this.tranService.errorMessage('no_payment_methods');
      } else {
        let convertResult = this.globService.ConvertKeysToLowerCase(result);
        for (let list of convertResult) {
          if (list.categorycode === 'C') {
            const temp = {
              data: list,
      ...
      this.ContactPaymentMethodComponent.emit('setMinHeight');
    }, async (error: any) => {
      await this.loading.dismiss();
  ...
  goBack() {
    // this.navCtrl.pop();
    this.ContactPaymentMethodComponent.emit('close');
  }
      ...
      this.cPMLNewService.ValidateCreditCardNumber(cardNum).subscribe(async (result: ValidCardNumberItem) => {
        ...
        if (result !== null && typeof (result) !== 'undefined') {
          if (result.ValidationStatus && result.ValidationStatus === 'Error') {
            this.checkCardState = true;
            this.stepForm1.controls['cardNumber'].setErrors({ 'incorrect': true });
            this.stepForm1.get('cardNumber').markAsTouched();
          } else {
            this.PayCode = result.PayCode;
            setTimeout(() => {
  ...
  submitTriggerBank() {
    document.getElementById('submitButtonBank').click();
  }
  ...
  async submitBank() {
    if (this.BankForm.valid) {
      ...
      await this.loading.present();
      this.cPMLNewService.CustomerBankPaymentMethodAdd(reqBody, this.ContactCode).subscribe(async (result: any) => {
        await this.loading.dismiss();
        this.cancelAddBank();
        this.getBothList();
