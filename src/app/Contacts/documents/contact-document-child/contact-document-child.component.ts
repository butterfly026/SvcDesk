...
import { ContactDocumentService } from './services/contact-document-service';
import { MAT_DIALOG_DATA, MatDialogRef } from '@angular/material/dialog';
import { SpinnerService } from 'src/app/Shared/services';
import { MatAlertService } from 'src/app/Shared/services/mat-alert.service';

@Component({
  selector: 'app-contact-document-child',
  templateUrl: './contact-document-child.component.html',
  styleUrls: ['./contact-document-child.component.scss'],
  providers: [DatePipe]
})
export class ContactDocumentChildComponent implements OnInit {
  ...
  constructor(
    ...
    private docService: ContactDocumentService,
    private formBuilder: FormBuilder,
    private matAlert: MatAlertService,
    public datePipe: DatePipe,
    public globService: GlobalService,
    private spinnerService: SpinnerService,
    private dialogRef: MatDialogRef<ContactDocumentChildComponent>,
  ...
  async createDocument() {
    ...
    if (this.datePipe.transform(reqParam.DateAuthored, 'yyyy-MM-dd') >
      ...
      this.tranService.errorMessage('Authored Date should be before today.');
      return;
    }
    await this.spinnerService.loading();
    this.docService.createDocument(this.globService.convertRequestBody(reqParam), this.ContactCode).subscribe(async (result: any) => {
