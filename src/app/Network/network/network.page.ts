...
import { Network } from 'src/app/model';
...
export class NetworkPage implements OnInit {
  ...
  @Output('NetworkComponent') NetworkComponent: EventEmitter<string> = new EventEmitter<string>();

  @ViewChild('NetworkGrid') NetworkGrid: jqxGridComponent;

  groupList: Array<Network> = [];
  ...
  async getList() {
    ...
    this.netService.getLoyaltyList(reqData).subscribe(async (result: Network[]) => {
  ...
  selectRow() {
    this.selectIndex = this.NetworkGrid.getselectedrowindex();
    this.selectedData = this.NetworkGrid.getrowdata(this.selectIndex);
  }
  ...
  exportData() {
    this.NetworkGrid.exportview('xlsx', 'Network List');
  }
  ...
  setGridData() {
    ...
    this.NetworkGrid.updatebounddata();
    this.source.localdata.length = 0;
    if (this.availDeleteState) {
      this.NetworkGrid.clearselection();
    }
  ...
  openDialog(): void {
    const dialogRef = this.dialog.open(NetworkDetailPage, {
      // width: '250px',
  ...
  goBack() {
    this.NetworkComponent.emit('close');
  }
