...
import {
    ...
    AccountAddressType, 
    AccountAddressUsageHistoryItem, 
    CountryDetail, 
    GetAccountAddressUsageResponse, 
    PostCodeDetail, 
    AccountAddressUpdateRequestBody
} from '../models';
...
export class AccountAddressService {
    ...
    getAccountAddressUsage(ContactCode: number): Observable<GetAccountAddressUsageResponse> {
        const headers = new HttpHeaders()
            .set('Authorization', this.tokens.AccessToken)
            .append('Content-Type', 'application/json');
        ...
        return this.httpclient.get<GetAccountAddressUsageResponse>(
            this.config.NewAPIEndPoint + `/Contacts/${ContactCode}/Addresses/Usage/`,
            { headers, params }
        );
    }

    updateAccountAddressUsage(reqData: AccountAddressUpdateRequestBody, ContactCode: string): Observable<void> {
        const headers = new HttpHeaders()
            .set('Authorization', this.tokens.AccessToken)
            .append('Content-Type', 'application/json');
    ...
    getAccountAddressUsageHistory(ContactCode: string): Observable<AccountAddressUsageHistoryItem[]> {
        const headers = new HttpHeaders()
            .set('Authorization', this.tokens.AccessToken)
            .append('Content-Type', 'application/json');
        ...
        return this.httpclient.get<AccountAddressUsageHistoryItem[]>(
            this.config.NewAPIEndPoint + `/Contacts/${ContactCode}/Addresses/UsageHistory/`,
            { headers, params }
        );
    }
